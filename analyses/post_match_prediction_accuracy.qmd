...
# Scouting Predictive Accuracy against Statbotics
...
saveRDS(new_df, file = "data/preds_comp.rds")
#this takes forever by the way so have fun waiting for 80 seconds
```

```{r}
new_df <- readRDS("data/preds_comp.rds")
...
calculate_priors <- function(data, match, event_code, opr, epas){
    ...
    priored <- prior_ridge(as.matrix(design), response, lambda, epas$start)[, 1]
    comparison <- cbind(epas$start, epas$preelim, opr, priored)
}

calculate_total_err <- function(raw, match_num, event_code, opr, epas) {
    ...
    probabilities <- probabilities |>
        ...
        mutate(
            priors = list(
                calculate_priors(data, exclude, event_code, opr, epas)
            ),
            predict_score =
...
# only need team number, start EPA, and post-quals EPA
epas <- data.frame(
    team = sapply(te, function(te){te$team}), 
    start = sapply(te, function(te){te$epa$stats$start}), 
    preelim = sapply(te, function(te){te$epa$stats$pre_elim})
)

loocv_df <- data.frame()

for (i in seq_along(up_to_match)){
    cat("i:", i, "\n")
    loocv_df <- rbind(
        loocv_df, 
        data.frame(
            event_code, match_limit = up_to_match[i], 
            mse = calculate_total_err(raw, up_to_match[i], 
                                      event_code, opr, epas)
        )
    )
}

write.csv(loocv_df, file = "data/probability_LOOCV.csv")
```
...
p <- ggplot(read.csv("data/probability_dchamp.csv"), aes(x = up_to_match, y = probability, label = probability)) + 
    ...
    labs(
        ...
        title = "DChamp TBA Data up to Match X vs. Prediction Accuracy %"
    )
...
ggplot(p, aes(x = match_limit, y = `mse`)) + 
    geom_point() + 
    labs(
