...
data <- raw %>%
  ...
  summarize(
    ...
    auto_coral_lower = sum(ifelse(acns!=0, 1*(ifelse(achs==1, 3, ifelse(achs==2, 4,
                              ifelse(achs==3, 6, 7))))+(acns-1)*2, 0)/n()),
    auto_coral_higher = sum(ifelse(acns!=0, (acns*(ifelse(achs==1, 3, ifelse(achs==2, 4,
                              ifelse(achs==3, 6, 7))))), 0))/n(),
    robot_net_total = sum(robot_net_score+robot_net_miss)/n(),
    robot_net_average = sum(
