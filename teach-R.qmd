...
-   Indentation
    ...
    -   We indent using tabs (a tab is 4 spaces, this requires a change from default RStudio)

-   one space (not two!) between fxn arguments ("add(a, b)"), parameter assignments ("na.rm = TRUE"), and operators ("a + b", "a == b")

-   Piping

    -   Use \|\> operator

    -   Pipes (either with "+" or "\|\>") require a new line
...
data <- raw |>
    ...
    filter(scout == scout_selected) |>
    select(scout, match) |> 
    rowwise() |>
    mutate(
        scouted = nrow(data[data$match < match, 1])
    )


ggplot(data, aes(x = match, y = scouted)) +
    geom_line(color = "#a9000a") + 
    theme_bw() + 
    geom_vline(xintercept = 69) + 
    geom_vline(xintercept = 122) + 
    geom_vline(xintercept = 226) +
    geom_vline(xintercept = 344) + 
    annotate("text", x = 35, y = 150, label = "Glen Allen", color = "#a7000a") +
    annotate("text", x = 95, y = 150, label = "Severn", color = "#a7000a") +
    annotate("text", x = 175, y = 150, label = "DChamps", color = "#a7000a") +
    annotate("text", x = 290, y = 100, label = "Newton", color = "#a7000a") + 
    annotate("text", x = 355, y = 100, label = "CRI", color = "#a7000a") + 
    labs(
        title = paste("Matches scouted by", scout_selected, "over Reefscape"),
        x = "Match",
        y = "# of Matches Scouted"
    )
```
