...
tldf <- mldf %>%
    ...
    summarize(
        ...
        #coral
        coral_pts_mean = round(
            mean(coral_l1_pts + coral_l2_pts + coral_l3_pts + 
                     coral_l4_pts, na.rm = TRUE), 
            digits = 2), 
...
viz <- tldf %>%
    filter(team == 449) %>%
    select(auto_pts_mean, endgame_pts_mean, algae_pts_mean, 
           algae_remove_pct, coral_pts_mean)
